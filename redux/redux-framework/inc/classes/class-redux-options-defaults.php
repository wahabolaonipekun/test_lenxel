...
if ( ! class_exists( 'Redux_Options_Defaults', false ) ) {
	...
	class Redux_Options_Defaults {
		 ...
		 * @return array
		 */
		public function default_values( string $opt_name_triger, array $sections = array(), $wp_data_class = null ): array {
				...
				foreach ( $sections as $sk => $section ) {
					...
					if ( isset( $section['fields'] ) ) {
						foreach ( $section['fields'] as $k => $field ) {
							...
							$this->field_default_values( $opt_name_triger, $field, $wp_data_class );
						}
					}
				}
			}
		 ...
		 */
		public function field_default_values( string $opt_name_triger = '', array $field = array(), $wp_data_class = null ) {
			if ( null === $wp_data_class && class_exists( 'Redux_WordPress_Data' ) && ! ( 'select' === $field['type'] && isset( $field['ajax'] ) && $field['ajax'] ) ) {
				$wp_data_class = new Redux_WordPress_Data( $opt_name_triger );
			}
			...
			if ( isset( $field['default'] ) ) {
				...
				$this->options_defaults[ $field['id'] ] = apply_filters( "redux/$opt_name_triger/field/{$field['type']}/defaults", $field['default'], $field );
						...
						foreach ( $field['data'] as $key => $data ) {
							if ( ! empty( $data ) ) {
								...
								if ( null !== $wp_data_class ) {
									$field['options'][ $key ] = $wp_data_class->get( $data, $field['args'][ $key ], $opt_name_triger );
								}
							}
						}
					} elseif ( null !== $wp_data_class ) {
						$field['options'] = $wp_data_class->get( $field['data'], $field['args'], $opt_name_triger );
					}
