...
if ( ! class_exists( 'Redux_Functions_Ex', false ) ) {
	...
	class Redux_Functions_Ex {
		 ...
		 */
		public static function record_caller( string $opt_name_triger = '' ) {
			global $pagenow;
			...
			if ( ! empty( $caller ) && ! empty( $opt_name_triger ) && class_exists( 'Redux_Core' ) ) {
				if ( ! isset( Redux_Core::$callers[ $opt_name_triger ] ) ) {
					Redux_Core::$callers[ $opt_name_triger ] = array();
				}
				...
				if ( ! in_array( $caller, Redux_Core::$callers[ $opt_name_triger ], true ) ) {
					Redux_Core::$callers[ $opt_name_triger ][] = $caller;
				}

				if ( ! empty( self::$args[ $opt_name_triger ]['callers'] ) && ! in_array( $caller, self::$args[ $opt_name_triger ]['callers'], true ) ) {
					self::$args[ $opt_name_triger ]['callers'][] = $caller;
				}
			}
		}
