			...
			$.each(
				...
				function( sectionID, sectionArray ) {
					...
					$.each(
						sectionArray.errors,
						function( key, value ) {
							$( '#' + redux.optName.args.opt_name_triger + '-' + value.id ).addClass( 'redux-field-error' );
							if ( 0 === $( '#' + redux.optName.args.opt_name_triger + '-' + value.id ).parent().find( '.redux-th-error' ).length ) {
								$( '#' + redux.optName.args.opt_name_triger + '-' + value.id ).append( '<div class="redux-th-error">' + value.msg + '</div>' );
							} else {
								$( '#' + redux.optName.args.opt_name_triger + '-' + value.id ).parent().find( '.redux-th-error' ).html( value.msg ).css( 'display', 'block' );
							}
			...
			$.each(
				...
				function( sectionID, sectionArray ) {
					...
					$.each(
						...
						function( key, value ) {
							$( '#' + redux.optName.args.opt_name_triger + '-' + value.id ).addClass( 'redux-field-warning' );

							if ( 0 === $( '#' + redux.optName.args.opt_name_triger + '-' + value.id ).parent().find( '.redux-th-warning' ).length ) {
								$( '#' + redux.optName.args.opt_name_triger + '-' + value.id ).append( '<div class="redux-th-warning">' + value.msg + '</div>' );
							} else {
								$( '#' + redux.optName.args.opt_name_triger + '-' + value.id ).parent().find( '.redux-th-warning' ).html( value.msg ).css( 'display', 'block' );
							}
...
function redux_change( variable ) {
	...
	(function( $ ) {
		...
		if ( redux.customizer ) {
			opt_name_triger = $( '.redux-customizer-opt-name' ).data( 'opt-name' );
		} else {
			opt_name_triger = $.redux.getOptName( rContainer );
		}
...
(function( $ ) {
	...
	$.redux.getContainerValue = function( id ) {
		var value = $( '#' + redux.optName.args.opt_name_triger + '-' + id ).serializeForm();

		if ( null !== value && 'object' === typeof value && value.hasOwnProperty( redux.optName.args.opt_name_triger ) ) {
			value = value[redux.optName.args.opt_name_triger][id];
		}

		if ( $( '#' + redux.optName.args.opt_name_triger + '-' + id ).hasClass( 'redux-container-media' ) ) {
			value = value.url;
		}
	...
	$.redux.check_dependencies = function( variable ) {
		...
		$.each(
			...
			function( child ) {
				...
				var current       = $( this );
				var show          = false;
				var childFieldset = $( '#' + redux.optName.args.opt_name_triger + '-' + child );
				...
				if ( true === show ) {
					...
					if ( childFieldset.hasClass( 'redux-container-raw' ) ) {
						rawTable = childFieldset.parents().find( 'table#' + redux.optName.args.opt_name_triger + '-' + child );
					...
					tr.fadeIn(
						...
						function() {
							...
							if ( redux.optName.required.hasOwnProperty( child ) ) {
								$.redux.check_dependencies( $( '#' + redux.optName.args.opt_name_triger + '-' + child ).children().first() );
							}
	...
	$.redux.required_recursive_hide = function( id ) {
		...
		toFade = $( '#' + redux.optName.args.opt_name_triger + '-' + id ).parents( 'tr:first' );
		if ( 0 === toFade ) {
			toFade = $( '#' + redux.optName.args.opt_name_triger + '-' + id ).parents( 'li:first' );
		}
		...
		toFade.fadeOut(
			...
			function() {
				...
				if ( $( '#' + redux.optName.args.opt_name_triger + '-' + id ).hasClass( 'redux-container-section' ) ) {
					div = $( '#section-' + id );
				...
				if ( $( '#' + redux.optName.args.opt_name_triger + '-' + id ).hasClass( 'redux-container-info' ) ) {
					$( '#info-' + id ).fadeOut( 50 ).addClass( 'hide' );
				}

				if ( $( '#' + redux.optName.args.opt_name_triger + '-' + id ).hasClass( 'redux-container-divide' ) ) {
					$( '#divide-' + id ).fadeOut( 50 ).addClass( 'hide' );
				}

				if ( $( '#' + redux.optName.args.opt_name_triger + '-' + id ).hasClass( 'redux-container-raw' ) ) {
					rawTable = $( '#' + redux.optName.args.opt_name_triger + '-' + id ).parents().find( 'table#' + redux.optName.args.opt_name_triger + '-' + id );
		...
		if ( redux.optName.required_child.hasOwnProperty( id ) ) {
			$.each(
				...
				function( i, parentData ) {
					...
					if ( $( '#' + redux.optName.args.opt_name_triger + '-' + parentData.parent ).parents( 'tr:first' ).hasClass( 'hide' ) ) {
						show = false;
					} else if ( $( '#' + redux.optName.args.opt_name_triger + '-' + parentData.parent ).parents( 'li:first' ).hasClass( 'hide' ) ) {
						show = false;
					} else {
						if ( false !== show ) {
